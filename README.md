...
- **FCast Foundation (in development)** — A new `:fcast` module ships the full [FCast](https://fcast.org) protocol — codec for all 19 v3 opcodes, mDNS discovery, a coroutine-based sender client, and a foreground receiver service — so SpatialFin can act as both an FCast sender (push the current Jellyfin stream to any FCast-capable TV/PC) and receiver (let a phone push YouTube/HLS/DASH URLs straight into the immersive XR player). The protocol layer is unit-tested and wired into the player action layer; the cast button, receiver picker, and arbitrary-URL playback path are landing in upcoming releases.
