...
export class BrowserApp {
  ...
  private renderDetails(item: JellyfinItem, versions: JellyfinItem[] = [], seasons: JellyfinItem[] = [], nextUp?: JellyfinItem) {
    ...
    this.content.innerHTML = `<section class="detail-page"><button class="back-button" type="button" data-tooltip="Back" aria-label="Back"><span aria-hidden="true">←</span><span class="sr-only">Back</span></button><div class="detail-backdrop"></div><div class="detail-copy" style="position: relative; z-index: 10;"><p class="eyebrow">${this.escape(item.Type ?? 'Video')}</p><h1>${this.escape(item.Name)}</h1><p class="detail-meta">${this.escape(metadata(item))}</p>${pillsHtml}<p style="margin-top: 1rem;">${this.escape(item.Overview ?? 'No synopsis is available.')}</p>${versionSelectHtml}<div class="browser-hero-actions-slot"></div></div><div class="detail-extra-rows" style="position: relative; z-index: 1; padding: 0 clamp(32px, 6vw, 88px) 40px;">${nextUpHtml}${seasonsHtml}${castHtml}</div></section>`;
