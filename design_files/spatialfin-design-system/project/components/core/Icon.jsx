...
const CUSTOM_ICONS = {
  cast: {
    ...
    paths: [
      ...
      "m 101.033,814.922 c -24.197,-30.972 -9.192,-73.703 28.328,-81.618 25.509,-5.381 51.102,11.103 57.386,36.962 7.923,32.602 -16.481,62.997 -50.011,61.916 -14.14,-0.456 -26.028,-6.346 -35.703,-17.26 m 55.668,-29.927 c 0.04,-0.829 0.118,-1.659 0.113,-2.488 -0.064,-10.512 -8.444,-19.162 -18.421,-19.026 -9.921,0.135 -18.275,9.13 -18.09,19.476 0.152,8.529 7.183,16.622 15.381,17.705 10.082,1.333 17.918,-4.218 21.017,-15.667 z",
    ],
  },
};
...
export function Icon({ name, size = 20, strokeWidth = 2, color = "currentColor", style = {} }) {
  ...
  if (custom) {
    const inner = custom.paths.map((d, i) => <path key={i} d={d} />);
    return (
      <span aria-hidden="true" style={{ display: "inline-flex", width: size, height: size, color, flexShrink: 0, ...style }}>
        <svg width={size} height={size} viewBox={custom.viewBox || "0 0 24 24"} fill="currentColor" xmlns="http://www.w3.org/2000/svg">
          {custom.transform ? <g transform={custom.transform}>{inner}</g> : inner}
        </svg>
      </span>
    );
  }
