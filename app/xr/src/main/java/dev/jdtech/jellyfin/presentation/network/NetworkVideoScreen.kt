...
private fun NetworkVideoScreenLayout(
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            XrBrowseHeader(
                title = state.item?.name ?: stringResource(CoreR.string.title_network),
                onBackClick = onBack,
            )
        }
                    ...
                    if (item.runtimeTicks > 0) {
                        Text(
                            text = stringResource(
                                ...
                                (item.runtimeTicks / 10000L / 60000L).toInt(),
                            ),
                            style = MaterialTheme.typography.titleMedium,
                        )
                    }

                    item.ratings.firstOrNull()?.let { rating ->
                        Text(
                            text = "TMDB ${rating.value}",
                            style = MaterialTheme.typography.titleMedium,
                        )
                    }
