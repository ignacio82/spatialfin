...
| `:sendspin` | SendSpin receiver service backed by `sendspin-jvm` for clock sync, buffering, multi-server arbitration, and Android `AudioTrack` PCM rendering. Connection topology: when an MA server URL is known the service **dials out** to `ws(s)://<ma>/sendspin` (official-MA-app style; a 5 s dial loop bounds Wi-Fi-blip recovery and registration is instant, so MA's universal-player wrapper link survives across plays) and skips mDNS advertising; the legacy listen-on-8928 (+1 on debug) + mDNS-advertise path remains for unconfigured receivers and as an automatic fallback after ~30 s of failed dials. <!-- updated 2026-06-12: outbound dial replaces server-initiated handshake when the MA URL is known — fixes "MA says playing but silent" after Wi-Fi blips and the ~15 s wrapper re-link wait after receiver restarts --> |
