...
| `:core:ui` | Shared **app-level** UI foundation lifted out of `:app:unified`: the `SpatialFinTheme` / `spacings` / shapes (`dev.spatialfin.presentation.theme`), the cross-feature Compose components `ErrorDialog` / `BaseDialog` / `XrConfirmDialog` / `FinishSetupCard` (`dev.jdtech.jellyfin.presentation.components`), the presentation utils — safe padding, parallax, `LocalOfflineMode`, adaptive grid cells (`dev.jdtech.jellyfin.presentation.utils`), and the **leaf browse primitives** used by film/network/local/setup — `ItemPoster`(+`Direction`), `ItemHeader`, `InfoText`, `OverviewText`, `ErrorCard`, `DetailMetadataRow`, `XrBrowseHeader`(+`BrowseHeaderAction`) in package `dev.jdtech.jellyfin.presentation.film.components`. Depends on `:core` and `:data` (the `SpatialFin*` models live in `:data`). Feature UI modules depend on this so screens stop reaching back into `:app:unified`. Packages were preserved on the move, so consumer imports are unchanged. **Split-package note:** the rest of the `…film.components` kit (badges, `ItemCard`, `RatingsRow`, home/detail/download/Seerr pieces) still lives in `:app:unified` under the *same* package — only same-package leaf components with no back-references were lifted. When moving more of the kit, check **same-package calls** (no import line) in addition to imports, e.g. `ItemCard`→`*Badge`/`ProgressBar`, `RatingsRow`→`RatingBadge`(+`Rating` from `:data`). <!-- added 2026-06-26: core:ui extraction (phase 1) + browse primitives (phase 2a) --> |
