...
- **XR `SurfaceEntity` hierarchy rule (alpha15/DP4)** — parenting the video surface below the movable/scaled root caused black output on Galaxy XR. The working path keeps `SurfaceEntity` attached directly to `activitySpace` and mirrors root pose/scale to that independent surface; the current root is a generic SceneCore `Entity`, created via `Entity.create`, solely for movement and Compose overlays. <!-- updated 2026-05-25: Kotlin-first Entity API and verified sibling-surface layout -->
- **XR player move/input arbitration** — controls, dialogs, skip actions, and next-episode prompts render on a projected UI root in front of the movable video root. Keep the video's `MovableComponent` detached while any interactive overlay is visible; on Galaxy XR a repeated button select/drag can otherwise fall through and move the screen. An in-flight intentional move remains attached until it completes so showing controls at move start does not interrupt repositioning. <!-- added 2026-05-25: prevent controls from initiating video movement -->
