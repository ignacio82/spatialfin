...
>
> Current version (always re-read `buildSrc/src/main/kotlin/Versions.kt` if in doubt): **2.6.0 (91)**, `compileSdk 37`, `targetSdk 35`, `minSdk 31`, JDK 21. The `tv` flavor uses `APP_CODE + 1_000_000` (currently `1000091`) — see [Play Track Bundles](#play-track-bundles).
...
```kotlin
const val APP_CODE = 91        // monotonically increasing integer
const val APP_NAME = "2.6.0"   // semver
```
...
| Split-A/V (XR video + TV audio) | `fcast/.../protocol/SplitAvMetadata.kt` (wire schema, embedded under `metadata.custom.splitAv`) + `app/unified/.../fcast/session/SplitAvController.kt` + `SplitAvPolicy.kt` (pure drift-correction policy, `SplitAvPolicyTest`) + `player/core/.../splitav/SplitAvVideoMaster.kt` (interface the local player implements; XR Activity binds via `SplitAvVideoBridge`). TV is the audio clock leader (PlaybackUpdate beacons at 10 Hz), XR follows via speed-nudge / hard-seek tiers. `RememberedReceiversStore` carries `audioLatencyMs` per receiver. <!-- added 2026-05-10 -->
