...
| `:core` | Shared models, LLM model manager, download workers, sync workers, plus the base `core.presentation.*` Compose surface (dummy previews, color/spacing tokens, a few components). Also the **form-factor primitive** `DeviceClass` (`dev.spatialfin.unified`, package preserved) — the `XR`/`TV`/`PHONE` enum + `detectDeviceClass()` + `DeviceClassCapabilities`, relocated from `:app:unified` so feature/shell/fcast modules can branch on form factor without an `:app` dep. <!-- updated 2026-06-26: DeviceClass relocated to :core (modularization phase 3 substrate) --> |
| `:core:ui` | Shared **app-level** UI foundation lifted out of `:app:unified`: the `SpatialFinTheme` / `spacings` / shapes (`dev.spatialfin.presentation.theme`), the cross-feature Compose components `ErrorDialog` / `BaseDialog` / `XrConfirmDialog` / `FinishSetupCard` (`dev.jdtech.jellyfin.presentation.components`), the presentation utils — safe padding, parallax, `LocalOfflineMode`, adaptive grid cells (`dev.jdtech.jellyfin.presentation.utils`), and the **shared browse primitives** used by film/network/local/setup (package `dev.jdtech.jellyfin.presentation.film.components`): `ItemPoster`(+`Direction`), `ItemHeader`, `InfoText`, `OverviewText`, `ErrorCard`, `DetailMetadataRow`, `XrBrowseHeader`(+`BrowseHeaderAction`), `ItemCard` and its badge cluster (`BaseBadge`, `Downloaded`/`Downloading`/`ItemCount`/`PlayedBadge`, `ProgressBar`), and `RatingsRow`+`RatingBadge`. Also hosts the **shared on-device LLM management cards** (`dev.jdtech.jellyfin.presentation.ai`: `BeamGemmaManagementCard`, `BeamAiCoreManagementCard` — flavor-agnostic despite the `Beam` prefix) rendered by both the Beam and TV settings hubs; they take their managers/state as plain params (the Hilt `BeamLlmEntryPoint` accessor stays app-side in each shell), so this adds a `:settings` dep (download manager + `AppPreferences`) on top of `:core` (`AICoreStatus`). Depends on `:core`, `:data` (the `SpatialFin*` and `Rating` models live in `:data`), and `:settings`. Feature UI modules depend on this so screens stop reaching back into `:app:unified`. Also hosts the **`PlayRequest` playback seam** (`dev.jdtech.jellyfin.presentation.player.PlayRequest`, a sealed `LibraryItem`/`LocalMedia`/`NetworkMedia`/`UniversalMedia`) — browse screens emit a `PlayRequest` through an `onPlay` lambda instead of touching `:player:xr`. Packages were preserved, so consumer imports are unchanged. **Split-package note:** the *rest* of the `…film.components` kit (home/detail/download/Seerr/Server pieces, `EpisodeCard`, `SearchBar`, `HomeView`, …) still lives in `:app:unified` under the *same* package — both modules contribute to it and it resolves cross-module for public composables. When moving more of the kit, check **same-package calls** (no import line) in addition to imports — that is how the `ItemCard`→`*Badge` and `RatingsRow`→`RatingBadge` coupling was found. <!-- added 2026-06-26: core:ui extraction (phase 1) + browse primitives incl ItemCard cluster (phase 2a) + shared LLM mgmt cards (phase 3b) --> |
