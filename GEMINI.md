...
- **`core/.../work/DownloadIntegrityWorker.kt`** — sweeps every PRIMARY task with status `STATUS_SUCCESSFUL`, validates `File(finalPath).exists() && length() == totalBytes` (AES-CTR keeps ciphertext at the same length as plaintext, so the size check is valid even with content encryption on), and on mismatch deletes any leftover bytes, flips the source row's path back to the `.download` tempPath (so `isDownloaded()` reflects "not yet downloaded"), marks the task PENDING, and re-enqueues `ResumableDownloadWorker` with `ExistingWorkPolicy.KEEP`. Triggered from `UnifiedApplication.onCreate` and from `DownloadsViewModel.init` via `Downloader.verifyDownloads()`. Subtitle tasks are not yet swept — embedded subs fall back at playback so they're non-blocking. <!-- added 2026-05-03 -->
