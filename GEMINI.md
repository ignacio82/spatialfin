...
| `:core:ui` | Shared **app-level** UI foundation lifted out of `:app:unified`: the `SpatialFinTheme` / `spacings` / shapes (`dev.spatialfin.presentation.theme`), the cross-feature Compose components `ErrorDialog` / `BaseDialog` / `XrConfirmDialog` / `FinishSetupCard` (`dev.jdtech.jellyfin.presentation.components`), and the presentation utils — safe padding, parallax, `LocalOfflineMode`, adaptive grid cells (`dev.jdtech.jellyfin.presentation.utils`). Depends on `:core`. Feature UI modules depend on this so screens stop reaching back into `:app:unified` for theme/dialogs. Packages were preserved on the move, so consumer imports are unchanged. <!-- added 2026-06-26: core:ui extraction (modularization phase 1) --> |
