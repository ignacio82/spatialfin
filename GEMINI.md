...
| FCast session manager (global) | `app/unified/.../fcast/session/FCastSessionManager.kt` (+ `FCastSessionUi.kt`, `PlaybackLauncher.kt`, `di/FCastModule.kt`) | Hilt `@Singleton` that wraps `FCastCastingController` so a cast survives Activity boundaries. Owns the global picker visibility (`pickerVisible`), the picked-receiver state (`pickedReceiver`), `JellyfinRepository`-backed URL resolution for `castSpatialItem(item)`, the discovery cache (1.5 s opportunistic scan, 30 s freshness window), and the `lastUsedFCastReceiverHostPort` pre-select hint. `FCastCastIconButton` / `FCastGlobalPickerHost` / `FCastMiniController` are the three composables form-factor roots mount. `launchPlayback(...)` is the play-tap interceptor: routes to FCast when `hasCastIntent()` is true, else falls through to the local-player intent. <!-- added 2026-05-08: session-scoped cast (Google Cast UX) --> |
