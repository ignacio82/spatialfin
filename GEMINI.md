...
- **XR player move/input arbitration** — controls and video use separate projected roots. Leave `MovableComponent` available so hidden-panel reveal and deliberate video repositioning continue to work; during an active pointer gesture inside `ControlPanelUI`, temporarily detach movement so a button press or drag cannot be interpreted as a video grab, then reattach on pointer release. The control `SpatialPanel` retains its established resize policy. <!-- updated 2026-05-25: scope movement suppression to active controls input -->
