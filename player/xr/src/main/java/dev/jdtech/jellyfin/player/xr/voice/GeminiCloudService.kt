        ...
        if (apiKey.isBlank()) {
            val status =
                GeminiCloudStatus(
                    ...
                    usedModel = false,
                    model = MODEL,
                    details = "Cloud API key missing",
                )
            Timber.d("GEMINI: cloud %s unavailable: %s", reason, status.details)
            return@withContext GeminiCloudTextResult(text = null, status = status)
        }
